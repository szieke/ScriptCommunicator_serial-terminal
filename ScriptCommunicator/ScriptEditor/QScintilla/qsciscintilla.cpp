...
#ifdef Q_OS_LINUX
void QsciScintilla::dragEnterEvent(QDragEnterEvent *event)
{
    getMainWindow()->dragEnterEvent(event);
}

void QsciScintilla::dropEvent(QDropEvent *event)
{
    getMainWindow()->dropEvent(event);
}
#endif
// The dtor.
...
void QsciScintilla::handleCharAdded(int ch)
{
    ...
    //If enter is pressed and the last line ends with a '{' then
    //'\n}' is added.
    if(ch == '\n')
    {
        int line;
        ...
        int openAboveBrackets = 0;
        int closedBelowBrackets = 0;
        int lineOfFirstOpenBracket = -1;

        //Check the previous text for brackets.
        while(currentLine >= 0)
        {
            linePos = count;
            while(linePos >= 0)
            {
                ...
                else if(buffer[linePos ] == '{')
                {
                    openAboveBrackets++;
                    if(lineOfFirstOpenBracket == -1)
                    {
                        lineOfFirstOpenBracket = currentLine;
                    }
                }
                linePos--;
            }
            currentLine--;
            count = SendScintilla(SCI_GETLINE, currentLine, buffer);
        }

        currentLine = line;
        count = SendScintilla(SCI_GETLINE, currentLine, buffer);
        if(openAboveBrackets > 0)
        {

            //Check the following text for brackets.
            ...
            if(openAboveBrackets > closedBelowBrackets)
            {
        ...
        {
            SendScintilla(SCI_ADDTEXT, 2, "\n}");
            setIndentation(line + 1, indentation(lineOfFirstOpenBracket));
...
*      The wheel event.
*/
void QsciScintilla::wheelEvent(QWheelEvent* event)
{
    if((Qt::ControlModifier & QApplication::keyboardModifiers()) != 0)
    {
        if (event->delta() > 0)
        {
    ...
    {
        QsciScintillaBase::wheelEvent(event);
    }
}
