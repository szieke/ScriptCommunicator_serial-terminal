...
function sendData()
{
	if(!g_stopSending)
	{
		...
		g_interface.i2cMasterReadWrite(0, 0, sendData.length, sendData);
		UI_Counter1.setText("counter: " + g_counter);	
		g_interface.setOutput(3, false);
		...
		{
			scriptThread.appendTextToConsole("i2cMasterReadLastReceivedData failed");
		}
	}
}
...
if(scriptThread.addToolBoxPagesToMainWindow(UI_ToolBox))
{
	...
	else
	{
		...
		g_interface.changePinConfiguration(2, true, true);
		g_interface.changePinConfiguration(3, false);
